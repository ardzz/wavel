...
namespace Ardzz\Wavel\Cores\Plugins;
...
class VCard
{
     ...
     * @throws VCardException
     */
    public function addBirthday(string $date): VCard
    {
        $this->setProperty(
            'birthday',
            'BDAY',
     ...
     * @throws VCardException
     */
    public function addCompany(string $company, $department = ''): VCard
    {
        $this->setProperty(
            'company',
     ...
     * @throws VCardException
     */
    public function addEmail(string $address, $type = ''): VCard
    {
        $this->setProperty(
            'email',
     ...
     * @throws VCardException
     */
    public function addJobtitle(string $jobtitle): VCard
    {
        $this->setProperty(
            'jobtitle',
     ...
     * @throws VCardException
     */
    public function addLabel(string $label, $type = ''): VCard
    {
        $this->setProperty(
            'label',
            'LABEL' . ($type !== '' ? ';' . $type : ''),
     ...
     * @throws VCardException
     */
    private function addMedia(string $property, string $url, $include = true, string $element)
    {
        $mimeType = null;
     ...
     * @throws VCardException
     */
    private function addMediaContent(string $property, string $content, string $element)
    {
        $finfo = new \finfo();
     ...
     * @throws VCardException
     */
    public function addPhoneNumber(string $number, $type = ''): VCard
    {
        $this->setProperty(
            'phoneNumber',
            'TEL' . (($type != '') ? ';' . $type : ''),
     ...
     * @throws VCardException
     */
    public function addLogo(string $url, $include = true): VCard
    {
        $this->addMedia(
            'LOGO',
            $url,
     ...
     * @throws VCardException
     */
    public function addLogoContent(string $content): VCard
    {
     ...
     * @throws VCardException
     */
    public function addPhoto(string $url, $include = true): VCard
    {
        $this->addMedia(
            'PHOTO',
            $url,
     ...
     * @throws VCardException
     */
    public function addPhotoContent(string $content): VCard
    {
        $this->addMediaContent(
            'PHOTO',
            $content,
     ...
     * @throws VCardException
     */
    public function addURL(string $url, $type = ''): VCard
    {
        $this->setProperty(
            'url',
            'URL' . (($type != '') ? ';' . $type : ''),
    ...
    public function buildVCard(): string
    {
        ...
        foreach ($properties as $property) {
            // add to string
            if ($property["key"] == "TEL"){
                $string .= $this->fold($property['key'] . ';waid=' . $this->escape($property['value']) . ":" . $this->escape($property['value']) . "\r\n");
            }else{
     ...
     */
    protected function chunk_split_unicode(string $body, $chunklen = 76, $end = "\r\n"): string
    {
        $array = array_chunk(
